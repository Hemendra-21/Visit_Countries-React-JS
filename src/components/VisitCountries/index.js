...
class VisitCountries extends Component {
  state = {countriesVisitedCount: 2, countriesList: initialCountriesList}
  ...
  onRemoveVisitedCountry = id => {
    ...
    this.setState(prevState => ({
      ...
      countriesVisitedCount: prevState.countriesVisitedCount - 1,
    }))
  }

  render() {
    const {countriesList, countriesVisitedCount} = this.state
    return (
      <VisitCountriesContainer>
          ...
          {countriesList.map(eachItem => {
            ...
            return (
              <ListItem key={eachItem.id}>
                <ListItemName>{eachItem.name}</ListItemName>
                {eachItem.isVisited === true ? (
                  ...
                  <VisitBtn
                    type="button"
                    onClick={() => {
                      const updatedList = countriesList.map(item => {
                      ...
                      this.setState(prevState => ({
                        ...
                        countriesVisitedCount:
                          prevState.countriesVisitedCount + 1,
                      }))
                    }}
                  >
                    Visit
            ...
            countriesList.map(
              eachCountry =>
                eachCountry.isVisited === true && (
                  <CountryItem
                    details={eachCountry}
                    onRemoveVisitedCountry={this.onRemoveVisitedCountry}
                    key={eachCountry.id}
                  />
                ),
            )
